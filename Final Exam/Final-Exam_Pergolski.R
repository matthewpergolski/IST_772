...
# 5. What variables predict whether or not a district's reporting was complete?
    ...
    # bayesian.model.5 <- lmBF(number ~ ., districts.lm.data)
    # summary(bayesian.model.5)
    
    # When comparing all variables to the 'DistrictComplete' attribute, we see that 2 variables have statistical
    # significance and may be sufficient predictors: (1) 'Enrolled' (p-value = 8.24e-11) and (2) 'TotalSchools (p-value = 2.20e-12).'
    ...
    # R-squared values are around 0.1, which does not suggest a strong model.
...
# date vaccines?
    ...
    bayesian.model.6 <- lmBF(PctUpToDate ~ WithoutDTP + WithoutMMR + PctBeliefExempt, data = districts.lm.data, 
...
# 7. What variables predict the percentage of all enrolled students with belief exceptions?
    ...
    bayesian.model.7 <- lmBF(PctBeliefExempt ~ WithoutDTP + WithoutHepB + PctUpToDate + PctFreeMeal, 
                             data = districts.lm.data, posterior = TRUE, iterations = 10000)
...
# question? 
      ...
      # Please see PDF Final Exam submission for the explanation/answer to this question.
